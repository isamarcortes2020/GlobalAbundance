# -*- coding: utf-8 -*-
"""
Created on Tue Feb 24 15:43:12 2026
...
nfiles = sorted(glob.glob("L:/US/a_Divided/chunk15/*.parquet"))###########change this



# Diagnostic prints to catch empty file lists early
print(f"Number of files found: {len(nfiles)}")
print(f"First few files: {nfiles[:3]}")

# ---------------------------
# Load LoadedData (single DF)
# ---------------------------
res1 = pyreadr.read_r("L:/TraitDataUpdated.rds")
LoadedData = list(res1.values())[0]     # pandas DataFrame
traits_df = pl.from_pandas(LoadedData)  # Changed from pl.DataFrame()

print(f"traits_df shape: {traits_df.shape}")

n_files = len(nfiles)

# Simplified loading using glob results directly (avoids index mismatch)
print("Loading parquet files...")
df_list_up = [pl.read_parquet(f) for f in tqdm(nfiles, desc="Loading files")]

print(f"First df shape: {df_list_up[0].shape if df_list_up else 'EMPTY LIST'}")
...
def assign_traits(
    ...
    if method == "match_species":
        ...
        for trait_name in tqdm(unique_traits, desc="  Assigning traits", leave=False):
...
def assign_traits_to_list(
    ...
    method: Literal["match_species", "any_species"] = "match_species",
    format: Literal["wide", "long"] = "wide",
    output_dir: str = "L:/US/combinedData/chunk15"
) -> list[pl.DataFrame]:
    """
    ...
    for i, df in tqdm(enumerate(df_list), total=len(df_list), desc="Processing dataframes"):
